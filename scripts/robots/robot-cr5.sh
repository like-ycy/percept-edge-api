...
DEFAULT_ROBOT_OS_CWD="/home/ubuntu/workspaces/percept-edge/ontology-core"
DEFAULT_ROBOT_OS_CMD="/usr/local/python3.10/bin/python3.10 /home/ubuntu/workspaces/percept-edge/ontology-core/robot_os.pyz runtime --mode homologous --log-level INFO"
...
check_active_uploads() {
  ...
  local has_active
  has_active=$(python3 -c "import sys,json; print(json.load(sys.stdin)['data']['has_active_upload'])" <<<"${response}" 2>/dev/null) || return 0

  if [ "${has_active}" = "True" ]; then
    local count
    count=$(python3 -c "import sys,json; print(len(json.load(sys.stdin)['data']['records']))" <<<"${response}" 2>/dev/null) || count="?"
    ...
    while [ ${wait_count} -lt "${max_wait}" ]; do
      ...
      has_active=$(python3 -c "import sys,json; print(json.load(sys.stdin)['data']['has_active_upload'])" <<<"${response}" 2>/dev/null) || break
...
usage() {
  cat <<'EOF'
Usage: ./scripts/robots/robot-cr5.sh [--env <test|prod>] [--mode <bilateral>]

Options:
  -e, --env      指定 API 使用的运行环境
  -m, --mode     启动模式: bilateral (默认, CR5 同构臂模式)
EOF
}
...
main() {
  ...
  case "${SELECTED_MODE}" in
    bilateral) ;;
    *)
      err "不支持的启动模式: ${SELECTED_MODE}，${ROBOT_NAME} 仅支持 bilateral"
      exit "${EXIT_PRECONDITION}"
      ;;
  esac
  log "启动模式: ${SELECTED_MODE}（CR5 仅启动 Robot OS 和 API）"
  ...
  require_file "${robot_os_cwd}/robot_os.pyz"
