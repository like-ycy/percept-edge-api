...
| 管理 | CLI | `uv run scripts/admin/release_collection_lock.py` | 查询/释放全局采集锁 |
| 上传 | POST | `/api/upload/start` | 单条上传 |
| 上传 | POST | `/api/upload/batch` | 批量上传 |
| 上传 | GET | `/api/upload/progress/{record_id}` | 上传进度 |
| 调试 | GET | `/debug/zeromq` | ZeroMQ 消费状态 |
...
- 开始采集时只检查 `robotos_collection` 是否有近期数据，跳过 `robotos_monitor` 和任务设备类型校验。
...
```bash
# 仅查看当前锁状态
uv run scripts/admin/release_collection_lock.py --status

# 强制解锁（写入操作者与备注）
uv run scripts/admin/release_collection_lock.py --operator wang --note "camera fixed" --force
```
