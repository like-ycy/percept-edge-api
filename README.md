...
2. **后台整理阶段**：停止采集后异步回放 raw spool，生成最终 `json/mp4`
...
output_dir/
  .capture/
    ...
    segment-000001.bin
    SEALED
  file_20260509153020_123456.json
  file_20260509153020_123456_wrist_left_rgb.mp4
  file_20260509153020_123456_wrist_left_depth.mp4
```

最终产物文件名前缀为 `file_{experiment_time}_{uuid6}`，其中
`experiment_time` 为采集开始时间（`YYYYMMDDHHMMSS`），`uuid6` 为去除横线后的
UUID 前 6 位。同一条采集数据的 JSON 与所有 MP4 共享同一个前缀；后台整理重试会复用
`.capture/manifest.json` 中记录的 `filename_prefix`。

JSON 文件直接使用 contracts 中的 `Episode.model_dump(exclude_none=True)` 结构写出，顶层为
`metadata` 和 `steps`，采集阶段不输出 `action`；`action` 由后处理程序根据 observation 生成。
相机视频文件名固定从 ZeroMQ `component_id` 推导：去掉 `camera_` 前缀后追加 `_rgb` 或 `_depth`。
...
的是该清单中的顶层成品 JSON/MP4 文件；`.capture/` 和 `writer_frame_counts.snapshot` 是内部文件，不属于上传对象。
